...
import { Badge, Button, DataTable, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@english-coach/ui";
  ...
  const columns: ColumnDef<KnowledgeItem>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="grid gap-1">
          ...
          <span className="text-xs leading-6 text-slate-400">{ellipsize(row.original.example ?? "No example", 120)}</span>
        </div>
      ),
      header: "Pattern",
    },
    ...
    <Card className="grid gap-5">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_13rem_13rem]">
        <div className="grid gap-2 text-sm text-slate-300">
          <span>Source</span>
          <Select
            onValueChange={(value: string) => queryState.setSource(value === "all" ? undefined : (value as KnowledgeItemSource))}
