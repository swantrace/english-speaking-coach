...
1. **If dependencies ARE declared** (e.g., `"@english-coach/types": "workspace:*"` in package.json), remove the `prebuild` script. Turbo's `dependsOn: ["^build"]` handles this automatically.
...
2. **If dependencies are NOT declared**, the `prebuild` exists because `^build` won't trigger without a dependency relationship. The fix is to:
   - Add the dependency to package.json: `"@english-coach/types": "workspace:*"`
...
{
  "dependencies": {
    "@english-coach/types": "workspace:*",
    "@english-coach/utils": "workspace:*"
  },
  "scripts": {
    "build": "next build"
  }
...
// WRONG - root turbo.json with many package-specific overrides
{
  "tasks": {
    "test": { "dependsOn": ["build"] },
    "@english-coach/web#test": { "outputs": ["coverage/**"] },
    "@english-coach/api#test": { "outputs": ["coverage/**"] },
    "@english-coach/utils#test": { "outputs": [] },
    "@english-coach/cli#test": { "outputs": [] },
    "@english-coach/core#test": { "outputs": [] }
  }
}
...
import { Button } from "@english-coach/ui/button";
```
